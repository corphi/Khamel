...
For the moment, see the example usage at the bottom of @khamel.php@.
...
Khamel indents using single spaces. If you want it to use double spaces, modify @Khamel::spaces()@ to double its parameter. The root element’s direct children (usually @<head>@, @<body>@) won’t get indented, because the root element’s output indent is -1. The static variable @Khamel::$root_indent@ controls this feature.
...
* Khamel uses whatever encoding you like. Just re-encode its file and make sure to announce it to the browser. But it defaults to UTF-8; and you should keep it as is.
